...
def build_multistream_backbone(
    ...
    if backbone == "eyes_only_mobilevitv2":
        from .eyes_only_mobilevitv2 import EyesOnlyMobileViTv2Gaze

        return EyesOnlyMobileViTv2Gaze(
            weights=weights,
            freeze_encoder=freeze_encoder,
            use_grid=use_grid,
            grid_size=grid_size,
    ...
    if backbone == "convnextv2_atto":
        from .convnextv2_atto_multistream import ConvNeXtV2AttoMultistream

        return ConvNeXtV2AttoMultistream(weights=weights, freeze_encoder=freeze_encoder,
                                         use_grid=use_grid, grid_size=grid_size)
    if backbone == "dinov2":
        from .dinov2_multistream import DINOv2Multistream

        return DINOv2Multistream(weights=weights, freeze_encoder=freeze_encoder,
                                 use_grid=use_grid, grid_size=grid_size)
    if backbone == "eva02":
        from .eva02_multistream import EVA02Multistream

        return EVA02Multistream(weights=weights, freeze_encoder=freeze_encoder,
                                use_grid=use_grid, grid_size=grid_size)
    if backbone == "eyes_only_eva02_tiny":
        from .eyes_only_eva02_tiny import EyesOnlyEVA02TinyGaze

        return EyesOnlyEVA02TinyGaze(weights=weights, freeze_encoder=freeze_encoder,
                                     use_grid=use_grid, grid_size=grid_size)
    if backbone == "eyes_only_convnextv2_atto":
    ...
    if backbone == "eyes_only_convnextv2_binocular":
        ...
        return EyesOnlyConvNeXtV2BinocularGaze(weights=weights, freeze_encoder=freeze_encoder,
                                               use_grid=use_grid, grid_size=grid_size)
    if backbone == "convnextv2_film":
        from .convnextv2_film import ConvNeXtV2FiLMMultistream

        return ConvNeXtV2FiLMMultistream(
            weights=weights,
            freeze_encoder=freeze_encoder,
            use_grid=use_grid,
            grid_size=grid_size,
    ...
    if backbone == "convnextv2_nano":
        from .convnextv2_multistream import ConvNeXtV2NanoMultistream

        return ConvNeXtV2NanoMultistream(
            weights=weights,
            freeze_encoder=freeze_encoder,
            use_grid=use_grid,
            grid_size=grid_size,
    ...
    if backbone == "convnextv2_dualenc":
        from .convnextv2_dualenc import ConvNeXtV2DualEncMultistream

        return ConvNeXtV2DualEncMultistream(
            weights=weights,
            freeze_encoder=freeze_encoder,
            use_grid=use_grid,
            grid_size=grid_size,
