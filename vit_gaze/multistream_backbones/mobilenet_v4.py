"""MobileNetV4-Conv-Medium multistream backbone for gaze estimation.

Shared MobileNetV4-Conv-Medium encoder (from timm, ~8M params, 1280-d feature)
over face + left eye + right eye + optional face-grid, features concatenated and
fed to an MLP head -- the same late-fusion recipe as MultiStreamViTGaze, with an
efficient mobile-optimised conv encoder. MobileNetV4 uses the Universal Inverted
Bottleneck (UIB) block and is tuned for fast on-device inference.

Note: timm's ``num_features`` (960) differs from the actual pooled output width
(1280, after the conv_head expansion), so the feature dim is detected with a
dummy forward pass.

This is distinct from the existing ``mobilenet_v3`` backbone, which requires the
grid (iTracker-style fusion head). MobileNetV4 here uses the optional-grid ViT
recipe and exposes forward_features + a trimmable Linear(.,2) head, so the
FiLM/LoRA meta-adapters and embedding-space SVR calibration all work.
"""
...
_MOBILENETV4_MODEL = "mobilenetv4_conv_medium"


def _build_mobilenetv4_encoder(weights: str) -> nn.Module:
    """Build a timm MobileNetV4 feature extractor (num_classes=0 -> pooled)."""
    import timm
    ...
    return timm.create_model(_MOBILENETV4_MODEL, pretrained=pretrained, num_classes=0)
...
class MobileNetV4Multistream(MultistreamBackboneBase):
    """Shared MobileNetV4-Conv-Medium over face + both eyes (+ grid) -> MLP head."""
