...
def process_video(
    ...
    # Some captures are stored rotated (e.g. phone video saved 90 deg CCW). The
    # frontal face/eye detectors need upright frames; rotate at read time.
    _ROT = {
        "none": None,
    ...
    if rotate not in _ROT:
        raise ValueError(f"rotate must be one of {list(_ROT)}, got {rotate!r}")
    rot_code = _ROT[rotate]
    ...
    # Seek to the calibration window start. Frame indices below are reported in
    # the original video's numbering (start_frame + offset).
    if start_frame > 0:
